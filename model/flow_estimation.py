...
class MultiScaleFlow(nn.Module):
    ...
    def calculate_flow(self, imgs, timestep, sf=None, mf=None):
        img0, img1 = imgs[:, :3], imgs[:, 3:6]
        B = img0.size(0)
        flow, mask = None, None

        if (sf is None) or (mf is None):
            sf, mf = self.feature_bone(img0, img1)
        for i in range(self.flow_num_stage):
            ...
            if flow != None:
                ...
                flow_, mask_ = self.block[i](
                    torch.cat([t*mf[-1-i][:B], (1-t)*mf[-1-i][B:],sf[-1-i][:B],sf[-1-i][B:]],1),
    ...
    def coraseWarp_and_Refine(self, imgs, sf, flow, mask):
        img0, img1 = imgs[:, :3], imgs[:, 3:6]
        warped_img0 = warp(img0, flow[:, :2])
        warped_img1 = warp(img1, flow[:, 2:4])
        c0, c1 = self.warp_features(sf, flow)
    ...
    def forward(self, x, timestep=0.5):
        ...
        for i in range(self.flow_num_stage):
            ...
            if flow != None:
                flow_d, mask_d = self.block[i]( torch.cat([t*mf[-1-i][:B], (1-t)*mf[-1-i][B:],sf[-1-i][:B],sf[-1-i][B:]],1),
                                                torch.cat((img0, img1, warped_img0, warped_img1, mask), 1), flow)
                flow = flow + flow_d
                mask = mask + mask_d
            else:
                flow, mask = self.block[i]( torch.cat([t*mf[-1-i][:B], (1-t)*mf[-1-i][B:],sf[-1-i][:B],sf[-1-i][B:]],1),
